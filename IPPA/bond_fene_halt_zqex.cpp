/* ----------------------------------------------------------------------
   ...
   bond_fene Modified from ../MOLECULE/bond_fene.cpp   to accept an
   additional rmax argument. Simulation
   halts if a fenehalt bond extends beyond the rmax length.

   See the README file in the top-level LAMMPS directory.
------------------------------------------------------------------------- */

#include "bond_fene_halt_zqex.h"
...
void BondFENEHALT::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if (rsq > rhalt2[type]) {
      ...
      error->one(FLERR,fmt::format("FENE bond longer than specified distance: time {} {}-{} {}<{}", update->ntimestep,atom->tag[i1],atom->tag[i2]
                                    , sqrt(rhalt2[type]), sqrt(rsq) ));
    }
...
void BondFENEHALT::allocate()
{
  ...
  memory->create(rhalt2,np1,"bond:rhalt2");
...
// ADded by zqex
  double rhalt_one = utils::numeric(FLERR,arg[5],false,lmp);
...
void BondFENEHALT::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    ...
    utils::sfread(FLERR, &rhalt2[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
...
double BondFENEHALT::single(int type, double rsq, int /*i*/, int /*j*/, double &fforce)
{
    if (rsq > rhalt2[type]) {
      error->one(FLERR,fmt::format("FENE bond longer than specified distance: time {} {}<{} in single", update->ntimestep, sqrt(rhalt2[type]), sqrt(rsq) ));
    }
...
void *BondFENEHALT::extract(const char *str, int &dim)
{
  dim = 1;
